...
% Date: 10 Jun 2002
% CVS: $Id: stage.tex,v 1.9 2002-06-11 03:31:18 inspectorg Exp $
...
\chapter{Introduction}
  ...
  \section{How to get Stage and related software}
   ...
   \noindent Official source releases, access to the CVS development
   tree, bug tracking, user mailing lists, etc.~ is available from the
   Sourceforge project management page:\\\\\indent \SFPAGE\\

   \noindent The current release is Stage-\VERSION. A tarball {\tt Stage-\VERSION.src.tgz} should be available at:\\\\ \indent http://sourceforge.net/project/showfiles.php?group\_id=42445\\

	Previous versions were significantly different and this manual
...
\chapter{Running Stage}
  ...
  \section{Command Line Arguments}
    ...
    \begin{xarg}{-q}Quiet mode - disables the status line output
    \end{xarg}
    ...
    Set the port number for the server used by Stage clients (such as Stage in client mode - see below). Default is 6601.
    \end{xarg}
...
\hline
\verb'unit_length' & \verb'["m", "cm", "mm"]' & Set the unit length to meters, centimeters
or millimeters. \\
\verb'unit_angle' & \verb'["degrees", "radians"]' & Set the unit angle to degrees or radians.\\
\hline
\end{tabularx}
\end{table}
...
The following table lists all of the passive entity types supported by
Stage.  See Section \ref{sec.ref.devices} for a list of supported {\em
devices}.
...
\verb'shape' & \verb'"rect"' or \verb'"circle"' & Entity shape; entities
...
Most of the devices described in this section are simulated versions
of the real devices supported by Player; see the Player User Manual
