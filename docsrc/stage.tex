...
% Date: 10 Jun 2002
% CVS: $Id: stage.tex,v 1.4 2002-11-11 03:09:45 rtv Exp $
...
or more models. Each model is implemented by code in the
{\tt <stage\_src>/src/models} directory.
...
inheriting properties from their parent type.  Thus, for example, all
passive objects (boxes, pucks, beacons and so on) are derived from a
generic \verb'entity' type.  Similary, all controllable objects
(sensors and actuators) are derived from a generic \verb'device' type,
which is in turn derived from the generic \verb'entity' type.
...
detail in the follwing pages. See Section \ref{sec.ref.devices} for a
...
\newpage
\subsection{The generic {\tt entity} type}
...
While this type and cannot be instantiated directly, its descendents
...
\verb'name' & \verb'"name string"' & A descriptive name for this entity; used
by the GUI.\\
...
\verb'id' & \verb'beaconid' & The beacon id returned by an LBD device
scanning this object. In the range 0-255\\
...
\subsubsection*{Description}
This is a base type for all device models. The properties specify how
the device is addressed through the Player interface. 
...
\newpage
\section{The {\tt idarturret} device}
...
Name & Values & Description \\
\hline
\verb'consume' &  \parbox{30mm}{\verb'"true"' \verb'"false"'} & If {\tt true}, the
...
% \verb'scan_rate' & \verb'float' & \\ this doesn't seem to do anything just now
\hline
\end{tabularx}
...
\newpage
\section{The {\tt fiducialfinder} device}
...
\newpage
\section{The {\tt blobfinder} device}
