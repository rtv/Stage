...
% Date: 10 Jun 2002
% CVS: $Id: stage.tex,v 1.5.2.2.2.1 2003-12-06 22:09:00 gerkey Exp $
...
\titlepage
 ...
 \end{tabular}
  ...
  \centerline{\large{(compatible with Player 1.4rc2)}}
  \vspace{2cm}
...
\section{Citations}
If you find Stage and Player useful in your work, we would greatly appreciate
...
The worldfile is given as the last argument when invoking Stage: \verb+$ stage <worldfile>+.
Stage searches for the specified file in these directories, in this order:

\begin{enumerate}
\item the current directory
\item the directory that contains the current world file
\item the directories listed in the STAGEPATH environment variable
\end{enumerate}
...
\verb'gps' & \verb'device' & GPS receiver; {\em disabled}\\
...
\newpage
\subsection{The {\tt gps} device ({\em disabled})}
...
\subsubsection*{Description}
{\em The {\tt gps} is temporarily disabled, pending implementation of a
proper GPS model, or at least one that returns GPS-style coordinates.}
