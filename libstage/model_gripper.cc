...
// CVS info:
//  $Source:
//  /home/tcollett/stagecvs/playerstage-cvs/code/stage/src/model_gripper.c,v $
//  $Author: thjc $
//  $Revision$
//
///////////////////////////////////////////////////////////////////////////
...
/**
   ...
   Gripper arms state, either "open" or "closed", and lift state, either "up" or
   "down"
*/
...
void ModelGripper::Update()
{
  ...
  default: printf("unknown gripper command %d\n", cmd);
  }
  ...
  if (start_paddle_position != cfg.paddle_position || start_lift_position != cfg.lift_position)
...
static bool gripper_raytrace_match(Model *hit, const Model *finder, const void *dummy)
{
...
void ModelGripper::UpdateContacts()
{
  ...
  lpz.y = (1.0 - cfg.paddle_position) * ((geom.size.y / 2.0) - (geom.size.y * cfg.paddle_size.y));
...
void ModelGripper::DataVisualize(Camera *cam)
{
  ...
  double bby = invp * ((geom.size.y / 2.0) - (geom.size.y * cfg.paddle_size.y));
