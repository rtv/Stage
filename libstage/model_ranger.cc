...
// static members
Option ModelRanger::Vis::showTransducers("Ranger transducers",
		"show_ranger_transducers", "", false);
Option ModelRanger::Vis::showArea("Ranger area", "show_ranger_ranges", "",
		true);
Option ModelRanger::Vis::showStrikes("Ranger strikes", "show_ranger_strikes",
		"", false);
...
void ModelRanger::Sensor::Load(Worldfile *wf, int entity)
{
	...
	wf->ReadTuple(entity, "noise", 0, 3, "lfa", &range_noise_const, &range_noise,
			&angle_noise);
...
void ModelRanger::Sensor::Update(ModelRanger *mod)
{
	...
	for (size_t t(0); t < sample_count; t++)
	{
		...
		float distortedAngle = ray.origin.a
				+ sample_incr * angle_noise * simpleNoise() * 0.5;
...
std::string ModelRanger::Sensor::String() const
{
	char buf[256];
	snprintf(buf, 256,
			"[ samples %u, range [%.2f %.2f] fov %.2f color [%.2f %.2f %.2f %.2f]",
			sample_count, range.min, range.max, fov, color.r, color.g, color.b,
			color.a);
...
void ModelRanger::Sensor::Visualize(ModelRanger::Vis *vis, ModelRanger *rgr,
		Canvas * canvas) const
{
	...
	{
		canvas->PushColor(Color::blue); // solid color
		glPointSize(2);
...
void ModelRanger::Vis::Visualize(Model *mod, Camera *cam, Canvas * canvas)
{
	...
	FOR_EACH (it, sensors)
		it->Visualize(this, ranger, canvas);
