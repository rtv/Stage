...
public:
  ...
  Bounds vbounds, wbounds;
  Bounds dvbounds, dwbounds;
  ...
  VelSpace( const unsigned int vcount, 
	    ...
	    const Bounds& vbounds,
	    const Bounds& wbounds,
	    const Bounds& dvbounds,
	    const Bounds& dwbounds
	    ) :
    vcount(vcount),
    wcount(wcount),
    vbounds(vbounds),
  ...
  {    
      ...
      for( unsigned int wi=0; wi<wcount; ++wi )
	{
	  ...
	  const double vrange = vbounds.max - vbounds.min;
	  const double wrange = wbounds.max - wbounds.min;
	  
	  const double vincr = vrange / (double)vcount;
	  const double wincr = wrange / (double)wcount;
	  
	  vel.v = vbounds.min + vi * vincr;
	  vel.w = wbounds.min + wi * wincr;	  
  ...
  {        
    ...
    const double v1max = std::min( v + dvbounds.max * interval, vbounds.max );
    const double v1min = std::max( v + dvbounds.min * interval, vbounds.min );    
    
    const double w1max = std::min( w + dwbounds.max * interval, wbounds.max );
    const double w1min = std::max( w + dwbounds.min * interval, wbounds.min );
    ...
    FOR_EACH( it, vels )
      {
	it->Evaluate( r, b, vbounds.max, wbounds.max, cloud);
	...
	for( unsigned int w=0; w<vcount; ++w )
	  {
	    ...
	    Color c = vel.inwindow ? Color( vel.score,0,0 ) : Color( 0,0,vel.score ); 
	    c.a = 0.5;
	    c.GLSet();
	    glRectf( v, w, v+1, w+1 );
	  }
