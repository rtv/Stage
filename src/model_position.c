...
stg_model_t* stg_model_test_collision_at_pose(stg_model_t* mod,
		stg_pose_t* pose, double* hitx, double* hity);
...
void position_init(stg_model_t* mod) {
	...
	data.integration_bias.x = drand48() * STG_POSITION_INTEGRATION_BIAS_MAX_X
			- STG_POSITION_INTEGRATION_BIAS_MAX_X / 2.0;

	data.integration_error.x = STG_POSITION_INTEGRATION_ERROR_X;

	data.integration_bias.y = drand48() * STG_POSITION_INTEGRATION_BIAS_MAX_Y
			- STG_POSITION_INTEGRATION_BIAS_MAX_Y / 2.0;

	data.integration_error.y = STG_POSITION_INTEGRATION_ERROR_Y;

	data.integration_bias.a = drand48() * STG_POSITION_INTEGRATION_BIAS_MAX_A
	...
	data.max_speed.x = 1.0;
	data.max_speed.y = 1.0;
	data.max_speed.a = 1.0;

	stg_model_set_data(mod, &data, sizeof(data));

#if ! INCLUDE_GNOME
	stg_model_add_property_toggles(mod, &mod->data, position_render_data, // called when toggled on
			NULL, position_unrender_data, // called when toggled off
			NULL, "positionlines", "position lines", FALSE );

	stg_model_add_property_toggles(mod, &mod->data, position_render_text, // called when toggled on
			NULL, position_unrender_text, // called when toggled off
			NULL, "positiontext", "position text", FALSE );
#endif
}
...
void position_load(stg_model_t* mod) {
	...
	if (wf_property_exists(mod->id, "drive")) {
		const char* mode_str = wf_read_string(mod->id, "drive", NULL );
	...
	if (wf_property_exists(mod->id, keyword)) {
		data->origin.x
				= wf_read_tuple_length(mod->id, keyword, 0, data->pose.x);
		data->origin.y
				= wf_read_tuple_length(mod->id, keyword, 1, data->pose.y);
	...
	if (wf_property_exists(mod->id, "max_speed")) {
		data->max_speed.x = wf_read_tuple_length(mod->id, "max_speed", 0,
				data->max_speed.x);
		data->max_speed.y = wf_read_tuple_length(mod->id, "max_speed", 1,
				data->max_speed.y);
	...
	if (wf_property_exists(mod->id, "wheelbase")) {
		data->wheelbase = wf_read_float(mod->id, "wheelbase", 1.0);
	}
	...
	if (wf_property_exists(mod->id, "odom_error")) {
		...
		{
			data->integration_bias.x = wf_read_tuple_length(mod->id,
					"odom_error", 0, data->integration_bias.x);
			data->integration_error.x = 0;
			data->integration_bias.y = wf_read_tuple_length(mod->id,
					"odom_error", 1, data->integration_bias.y);
			data->integration_error.y = 0;
			data->integration_bias.a = wf_read_tuple_angle(mod->id,
					"odom_error", 2, data->integration_bias.a);
			data->integration_error.a = 0;
		} else if (wf_tuple_count(mod->id, "odom_error") == 6) {
			data->integration_bias.x = wf_read_tuple_length(mod->id,
					"odom_error", 0, data->integration_bias.x);
			data->integration_error.x = wf_read_tuple_length(mod->id,
					"odom_error", 1, data->integration_error.x);
			data->integration_bias.y = wf_read_tuple_length(mod->id,
					"odom_error", 2, data->integration_bias.y);
			data->integration_error.y = wf_read_tuple_length(mod->id,
					"odom_error", 3, data->integration_error.y);
			data->integration_bias.a = wf_read_tuple_angle(mod->id,
					"odom_error", 4, data->integration_bias.a);
			data->integration_error.a = wf_read_tuple_angle(mod->id,
					"odom_error", 5, data->integration_error.a);
		} else
	...
	if (wf_property_exists(mod->id, "localization")) {
		const char* loc_str = wf_read_string(mod->id, "localization", NULL );
	...
	if (wf_property_exists(mod->id, "watchdog_timeout")) {
		data->watchdog_timeout = (int) rint(1e3 * wf_read_float(mod->id,
				"watchdog_timeout", -1.0));
	}
...
int position_update(stg_model_t* mod) {
	...
	case STG_POSITION_LOCALIZATION_ODOM: {
		...
		if (cmd->mode == STG_POSITION_CONTROL_VELOCITY && cfg->drive_mode
...
int position_render_data(stg_model_t* mod, void* enabled) {
	...
	if (!fig) {
		fig = stg_model_fig_create(mod, "position_data_fig", NULL,
	...
	if (mod->subs) {
		...
		stg_rtk_fig_arrow(fig, 0, 0, 0, geom->size.x / 2.0, geom->size.y / 2.0);
		...
		stg_rtk_fig_rectangle(fig, odom->pose.x, odom->pose.y, odom->pose.a,
				geom->size.x, geom->size.y, 0);
		stg_rtk_fig_arrow(fig, odom->pose.x, odom->pose.y, odom->pose.a,
...
int position_render_text(stg_model_t* mod, void* enabled) {
	...
	if (!fig) {
		fig = stg_model_fig_create(mod, "position_text_fig", NULL,
				STG_LAYER_POSITIONDATA );
		stg_rtk_fig_color_rgb32(fig, mod->color);
	}
	...
	if (mod->subs) {
		...
		char buf[256];
		snprintf(buf, 255, "%s[%.2f, %.2f, %.1f]\nv[%.2f,%.2f,%.1f] %s\n",
				odom->localization == STG_POSITION_LOCALIZATION_GPS ? "GPS"
						: "odom", odom->pose.x, odom->pose.y, odom->pose.a,
				vel->x, vel->y, vel->a, mod->stall ? "STALL" : "");
