...
/*   printf( "** REDRAW interval %d ms\n", win->redraw_interval ); */
  ...
  win->panx = wf->ReadTupleFloat(section, "center", 0, win->panx );
  win->pany = wf->ReadTupleFloat(section, "center", 1, win->pany );
  win->stheta = wf->ReadTupleFloat( section, "rotate", 0, win->stheta );
  win->sphi = wf->ReadTupleFloat( section, "rotate", 1, win->sphi );
  win->scale = wf->ReadFloat(section, "scale", win->scale );
...
/*     } */
  ...
  width = (int)wf->ReadTupleFloat(section, "size", 0, width );
  height = (int)wf->ReadTupleFloat(section, "size", 1, height );
...
void gui_save( stg_world_t* world )
{
  ...
  wf->WriteFloat( win->wf_section, "scale", win->scale );

  wf->WriteTupleFloat( win->wf_section, "center", 0, win->panx );
  wf->WriteTupleFloat( win->wf_section, "center", 1, win->pany );

  wf->WriteTupleFloat( win->wf_section, "rotate", 0, win->stheta  );
  wf->WriteTupleFloat( win->wf_section, "rotate", 1, win->sphi  );

  wf->WriteInt( win->wf_section, "fill_polygons", win->fill_polygons );
  wf->WriteInt( win->wf_section, "show_grid", win->show_grid );
  wf->WriteInt( win->wf_section, "show_bboxes", win->show_bboxes );
  ...
  for( GList* list = win->toggle_list; list; list = g_list_next( list ) )
	...
	(stg_property_toggle_args_t*)list->data;
      ...
      wf->WriteInt( win->wf_section, tog->path, 
		     ...
		     gpointer user_data )
{
  ...
  gboolean confirm = quit_dialog( (GtkWindow*)widget );
...
const char* gui_model_describe(  StgModel* mod )
{
  stg_pose_t pose;
  mod->GetPose( &pose );
...
void gui_model_display_pose( StgModel* mod, char* verb )
{
  char txt[256];
...
{
  gui_window_t* win = (gui_window_t*)calloc( sizeof(gui_window_t), 1 );
  world->win = win;
...
{
  static char clock[256];
  stg_world_clockstring( world, clock, 256 );
  gtk_label_set_text( world->win->clock_label, clock );
}
